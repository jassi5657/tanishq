...
export const AuthProvider = ({ children }) => {
    ...
    const register = async (formData) => {
        ...
        const checkUser = await fetch(`http://localhost:4000/user/addUser?email=${formData.email}`, { method: "GET" });
        if (checkUser.ok) {
            let user = await checkUser.json();
            if (user.length > 0) {
                setMessage("User already exist, please login");
            } else {
                const response = await fetch("http://localhost:4000/user/addUser", config);
    ...
    const login = async (formData) => {
        const response = await fetch(`http://localhost:4000/user/login?email=${formData.email}&password=${formData.password}`, { method: "GET" });
        if (response.ok) {
    ...
    const getUserOnLoad = async (email) => {
        const response = await fetch(`http://localhost:4000/user/login?email=${email}`, { method: "GET" });
        if (response.ok) {
