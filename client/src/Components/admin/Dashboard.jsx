import React from 'react'
import styled from 'styled-components'
import { IoHomeOutline } from "react-icons/io5";
import { AiOutlineDashboard } from "react-icons/ai";
import { LuLayoutDashboard } from "react-icons/lu";
import { FaBorderAll } from "react-icons/fa";
import { FaRegUserCircle } from "react-icons/fa";
import Header from '../Header';
import Navbar from '../Navbar';
import { Link } from 'react-router-dom';
const Dashboard = () => {
  return (
    <Container>
        <br />
        <br />
        {window.location.pathname!== '/admin'? <Navbar /> : null}
                <LeftSide>
            <div className="logo">

                <img src="data:image/svg+xml;base64," alt="" />
                    <h2>Tanishq Admin</h2>
            </div>
            <hr />
            <ul>
                <Link to="" style={{textDecoration:"none" ,color:"black"}}><li><IoHomeOutline  style={{fontSize:"22px"}}/> Home</li></Link>
                <Link to="/admin/addProducts" style={{textDecoration:"none" ,color:"black"}}> <li><AiOutlineDashboard/> Add Product</li></Link>
                <Link to="/order" style={{textDecoration:"none" ,color:"black"}}><li><LuLayoutDashboard/> Orders</li></Link>
                <Link to="/admin/products" style={{textDecoration:"none" ,color:"black"}}><li><FaBorderAll/> Products</li></Link>
                <Link to="/customers" style={{textDecoration:"none" ,color:"black"}}><li><FaRegUserCircle/> Customers</li></Link>
                

            </ul>






        </LeftSide>
    </Container>
  )
}


const Container = styled.div`
margin-top: 70px;
`;
const LeftSide = styled.div`
width: 25%;
/* background-color: rgb(11,42,73); */
/* background-color: red; */

.logo{
display: inline-flex;
    img{
        width: 80px;
        height: 52px;
        float: left;
    }

    h2{
        font-size: 17px;
        margin-top: 20px;
        margin-left: 60px;
    }

}
ul{
    list-style-type: none;
    height: 100vh;
}
li{
    text-align: justify;
    margin: 20px;
    font-size: 20px;
}
`;

export default Dashboard